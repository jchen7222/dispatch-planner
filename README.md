...
Consolidation cuts the booking from a naive 85 consignments to 37 — **56% fewer consignments** — with one honest exception: an order physically too big for any single service and indivisible. Nothing is dropped silently — every unplanned order carries a reason code, and every broken promise produces exactly one live notification per affected party.
...
| Oversize orders | Split Delivery VRP idea | split across consignments only when `divisible=True`; else exception | `packing.py` |
...
| Delay impact | **Lineage traversal + idempotent fan-out** | consignment → stops → orders → parties; scoped notification per party; supersede by (party × consignment), never duplicate | `delay.py` |
...
consignments used               18                  18
total distance km           3208.0              3340.9
total drive min               3208                3012
plan fingerprint      dba0effefab3        8d3b700116fb
degraded?                        -               False

measured bias (osrm/haversine drive-time ratio) per corridor:
  EAST   x0.82   (1 loads)     NE     x1.09   (3 loads)
  NORTH  x0.90   (4 loads)     SOUTH  x0.88   (4 loads)
  SW     x1.01   (3 loads)     WEST   x0.83   (3 loads)
```
...
Two departure policies, same orders, same carrier network:

- **Earliest departure** absorbs injected delays completely (OTIF 100%) — waiting time at stops doubles as buffer — but the long spans left a third of loads with no departure still open before its cutoff.
- **Just-in-time departure** (what the CP-SAT stage chooses when it helps) gets every load onto a departure — spans shrink toward the waitless minimum, so one departure absorbs several routes — but the buffer is gone: the same injection drops OTIF to ~84%.
...
- **Weight vs. cube:** which capacity binds depends on freight density — each consignment reports *weigh-out* or *cube-out*.
...
- **Waiting time is not handling time:** the handling budget counts pure collection minutes; the span a consignment occupies between `accept_from` and `cutoff` is modeled separately (and shrinks when departing later).
- **Delay math against the PROMISE, not the plan.** A consignment can run late and still deliver inside the window — that is a delay of zero and **no notification**. Getting this wrong spams parties about non-events (tested).
...
- **Supersede, never duplicate.** A consignment delayed twice updates its notification (idempotency key: party × consignment); the ledger keeps both — the audit trail shows what each party was told at every point (tested).
