"""Property tests: conservation, capacity, windows, HOS, splits, determinism, quality floor."""
import math
import pytest
from dispatch.generator import make_orders, make_drivers, FLEET
...
def test_conservation(result):
    ...
    orders, drivers, loads, exceptions, led = result
...
def test_hos_and_shifts(result):
    _, drivers, loads, _, _ = result
    dmap = {d.driver_id: d for d in drivers}
    per = {}
    for l in loads:
        if not l.driver_id:
            continue
        d = dmap[l.driver_id]
        assert d.shift_start <= l.depart_min and l.arrive_back_min <= d.shift_end
        per.setdefault(l.driver_id, []).append(l)
    for did, ls in per.items():
        assert sum(l.drive_min for l in ls) <= dmap[did].max_drive_min
...
def test_split_delivery_and_indivisible_exception():
    ...
    loads, exc = pack([big_div, big_ind], FLEET)
    alloc_loads = [l for l in loads if any(a.order_id == "BIG1" for a in l.allocations)]
    assert len(alloc_loads) >= 2, "divisible oversize must split across trucks"
...
def test_utilization_floor(result):
    ...
    orders, _, loads, _, _ = result
    from dispatch.metrics import naive_truck_count
    assert len(loads) <= 0.6 * naive_truck_count(orders, FLEET)
